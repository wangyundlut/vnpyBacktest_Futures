...
可以使用和实盘相同的代码进行回测。
基于VNPY ctaBacktesting的修改，数字货币的回测
主要修改如下：
合约未实现盈亏的计算规则：合约的未实现盈亏（多头）是用 (1/持仓均价-1/最新成交价)*合约张数*合约面值 来计算的
修改trading_result里面的数据和daily_result的计算方式

初始账户资金：5个或2个BTC

手续费的计算问题，千分之2，以交易价格换算成BTC的数量计算

滑点的计算问题，变成多花了多少个BTC


关于时间的处理：
由于交易所系统时间和实际需要计算的交易日期之间需要转换
比如，交易所是用国际标准时间，而实际我们收到的时间有可能是所在时区的时间
或者，交易所用的是当地的时间，而需要我们自己转换交易日期


bar时间称为系统时间
bar.date修改为成交日期：夜盘时间交易日转换，以及标准时间和datetime的转换问题
vtTradeObject对象里面增加一项tradeDate，用来记录成交的实际日期，统计每日收益每日结果用
engine里面增加一项self.date用来记录系统成交的日期
newtick newbar里面增加self.date的修改，以及updailyclose的修改
cross Limit order  ;cross Stop order里面修改trade 的成交datetime对象以及date对象
updateDailyclose里面的date数据修改

calculateDailyResult(self) 里面的trade date的修改
...
class BacktestingEngine(object):
    ...
    def __init__(self):
        ...
        self.capital = 1            # 回测时的起始本金（默认5个BTC）
        ...
        self.tick = None
        self.bar = None
        self.datetime = None      # 最新的时间，这里的概念是最新的bar的datetime时间
        self.tradedate = None     # 最新的成交日期，这里的概念是成交归属哪一个日期
    ...
    def runBacktesting(self):
        ...
        for d in self.dbCursor:
            data = dataClass()
            data.__dict__ = d
            # print("发送数据到引擎: " + data.datetime.strftime("%Y-%m-%d %H:%M:%S"))
            func(data)     
    ...
    def newBar(self, bar):
        ...
        self.tradedate = bar.tradedate
        
        self.crossLimitOrder()      # 先撮合限价单
        self.crossStopOrder()       # 再撮合停止单
        self.strategy.onBar(bar)    # 推送K线到策略中
        # 这里需要注意的是，把bar的交易日期提取出来，有可能因为时区或者夜盘产生交易日期于datetime的日期不同的情况
        self.updateDailyClose(bar.tradedate, bar.close)
    ...
    def newTick(self, tick):
        ...
        self.tradedate = tick.tradedate
        
        self.crossLimitOrder()
        self.crossStopOrder()
        self.strategy.onTick(tick)
        # 这里需要注意的是，把tick的交易日期提取出来，有可能因为时区或者夜盘产生交易日期于datetime的日期不同的情况
        self.updateDailyClose(tick.tradedate, tick.lastPrice)
        
    #----------------------------------------------------------------------
    def initStrategy(self, strategyClass, setting=None, name=None):
        """
        初始化策略
    ...
    def crossLimitOrder(self):
        ...
        for orderID, order in list(self.workingLimitOrderDict.items()):
            ...
            if buyCross or sellCross:
                ...
                # 将bar的tradedate传过来
                trade.tradedate = self.tradedate
    ...
    def crossStopOrder(self):
        ...
        for stopOrderID, so in list(self.workingStopOrderDict.items()):
            ...
            if buyCross or sellCross:
                ...
                order.status = STATUS_ALLTRADED
                order.orderTime = trade.datetime
    ...
    def sendOrder(self, vtSymbol, orderType, price, volume, strategy):
        ...
        order.orderID = orderID
        order.vtOrderID = orderID
        order.orderTime = self.datetime.strftime('%H:%M:%S')
    ...
    def runOptimization(self, strategyClass, optimizationSetting):
        ...
        for setting in settingList:
            ...
            d, result, result_List = self.calculateDailyStatistics()
            try:
    ...
    def calculateDailyStatistics(self, annualDays=240):
        """计算按日统计的结果"""
        # 这里python2和python3的不同，orderdict的keys（）对象，不支持indexable，但是可以明确的将它转换成list
    ...
    def showDailyResult(self, d=None, result=None):
        ...
        if d is None:
            self.calculateDailyResult()
            d, result, resultList = self.calculateDailyStatistics()
...
class TradingResult(object):
    ...
    def __init__(self, entryPrice, entryDt, exitPrice, 
        ...
        self.exitDt = exitDt            # 平仓时间datetime
        
        self.volume = volume    # 交易数量（+/-代表方向）
        
        # self.turnover = (self.entryPrice+self.exitPrice)*size*abs(volume)   # 成交金额
        self.turnover = (1/self.entryPrice + 1/self.exitPrice) * size * abs(volume)  # 成交金额，意思是成交了多少个BTC
        self.commission = self.turnover*rate                                # 手续费成本
        # self.slippage = slippage*2*size*abs(volume)                         # 滑点成本
        # 滑点成本
        # 平仓方向为负，说明开的是多仓
        if volume < 0:
            self.slippage = (1 / entryPrice - 1 / (entryPrice + slippage)) * size * abs(volume) + \
                            (1 / (exitPrice - slippage) - 1 / exitPrice) * size * abs(volume)
        else:
            self.slippage = (1 / (entryPrice - slippage) - 1 / entryPrice) * size * abs(volume) + \
                            (1 / exitPrice - 1 / (exitPrice + slippage)) * size * abs(volume)
        # self.pnl = ((self.exitPrice - self.entryPrice) * volume * size
        #            - self.commission - self.slippage)                      # 净盈亏
        self.pnl = ((1/self.entryPrice - 1/self.exitPrice ) * volume * size
                    - self.commission - self.slippage)  # 净盈亏
...
class DailyResult(object):
    ...
    def calculatePnl(self, openPosition=0, size=1, rate=0, slippage=0):
        ...
        try:
            self.positionPnl = self.openPosition * (1/self.previousClose - 1/self.closePrice) * size
        ...
        for trade in self.tradeList:
            ...
            self.tradingPnl += posChange * (1/trade.price - 1/self.closePrice) * size
            self.closePosition += posChange
            self.turnover += 1/trade.price * trade.volume * size
            # self.commission += trade.price * trade.volume * size * rate
            self.commission += 1/trade.price * trade.volume * size * rate
            # self.slippage += trade.volume * size * slippage
            self.slippage += trade.volume * size * (1/trade.price - 1/(trade.price + slippage))
